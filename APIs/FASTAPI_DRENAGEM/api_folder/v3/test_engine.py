...
def get_engine(model_file='v3/weights/model.onnx', engine_file='v3/weights/model.plan', fp16=True, verbose=True):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(EXPLICIT_BATCH) as network, builder.create_builder_config() as config,\
        ...
        network.get_input(0).shape = [ 1, 3, 1024, 1024] # verificar tamanho da imagem

        # builder engine
        engine = builder.build_engine(network, config)
        print("[INFO] Completed creating Engine!", engine_file)
...
def main():
    ...
    plan_path = 'v3/weights/model.plan'
    ...
    if os.path.exists(plan_path):
        ...
        if 'Using an engine plan file across different models of devices is not recommended and is likely to affect performance or even cause errors.' in stdout:
            ...
            os.remove('api_folder/v3/weights/model.plan')
