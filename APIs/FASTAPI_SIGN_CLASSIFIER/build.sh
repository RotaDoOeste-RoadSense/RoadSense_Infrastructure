docker build -t mato .
docker rm -f mato >/dev/null 2>&1 || true
...
docker run -it -v .:/app --gpus='"device=0"' --rm -p 8016:8016 --ipc=host --ulimit memlock=-1 --ulimit stack=67108864 --name mato mato bash
