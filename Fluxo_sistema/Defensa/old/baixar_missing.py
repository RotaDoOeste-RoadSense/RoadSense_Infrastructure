...
def convert_pano2cube_2022(panorama, cam):
    return panorama + f'_cam{cam}.jpg'
...
outdir = '/media/rdt/hd2/clone_2/defensas_outlier_v3'
...
#image_folder_cube = images_folder.replace('teste','hd1') + '/Cube_real_norte'
...
for defensa in tqdm(Defensas_detectadas):
    ...
    is_outlier = defensa.outlier
    if class_name == 'Metal' and is_outlier:
        image_filename_panoramic = session.query(ImageData.image_name).filter(ImageData.image_id == image_id).first()[0]
        #image_filename_cube = convert_pano2cube_2022(image_filename_panoramic, cam)
        ...
        save_path = os.path.join(outdir, f'{class_name}_{id_unique_defensa}_{image_filename_cube}')
